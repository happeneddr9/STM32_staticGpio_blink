...
/* Static Gpio */
template<uint32_t peripheral_addr>
class static_GpioBase {
	public:
		using GpioAddr_type = GPIO_TypeDef *;
		using GpioAddr_type_c = GPIO_TypeDef * const;

		// using GpioPin to initialize
		constexpr static_GpioBase(const GpioPin GP = NoPin) :
		mPin16(1u << GP.pin) {
			RCC->AHBENR |= RCC_AHBPeriph_GPIOA << static_cast<uint8_t>(GP.port);
		}

		// using Port number and pin number to initialize
		constexpr static_GpioBase(Port port, uint16_t pin) :
				mPin16(pin) {
			RCC->AHBENR |= RCC_AHBPeriph_GPIOA << static_cast<uint8_t>(port);
		}

		// it doesn't initialize the RCC.
		constexpr static_GpioBase(uint16_t pin) :
				mPin16(pin) {

		}

		static void RCC_config(Port port) {
			RCC->AHBENR |= RCC_AHBPeriph_GPIOA << static_cast<uint8_t>(port);
		}
		...
		finline auto getGPIOPort(void) const {
			return mPort;
		}

		finline uint8_t getPinNum(void) const {
			uint8_t mPin = firstSetBit(mPin16);
			return mPin;
		}
		...
		finline static_GpioBase& operator =(GPIO_Status s) {
			setState(static_cast<bool>(s));
			return *this;
		}

		finline static_GpioBase& operator =(bool status) {
			setState(status);
			return *this;
		}

		finline const static_GpioBase& operator =(GPIO_Status s) const {
			setState(static_cast<bool>(s));
			return *this;
		}

		finline const static_GpioBase& operator =(bool status) const {
			setState(status);
			return *this;
		}
		...
		// count the first '1' of mPin16, slow version.
		uint8_t pin16_to_8() const {
			uint8_t i = 0;
			uint16_t tmp = mPin16;
			for (; i < 0x10; ++i) {
				tmp >>= 1;
	...
	protected:
		static GpioAddr_type mPort;
		uint16_t mPin16;
};

template<uint32_t peripheral_addr>
typename static_GpioBase<peripheral_addr>::GpioAddr_type static_GpioBase<peripheral_addr>::mPort = GpioAddr_type(peripheral_addr);

using GpioA = static_GpioBase<GPIOA_BASE>;
using GpioB = static_GpioBase<GPIOB_BASE>;
using GpioC = static_GpioBase<GPIOC_BASE>;
using GpioD = static_GpioBase<GPIOD_BASE>;
using GpioE = static_GpioBase<GPIOE_BASE>;
using GpioF = static_GpioBase<GPIOF_BASE>;
